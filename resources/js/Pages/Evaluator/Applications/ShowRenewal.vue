...
const saveRequirementStatus = (status) => {
    ...
    router.post(route('evaluator.applications.renewal.evaluate', application.value.id), {
...
const submitApproval = () => {
    ...
    router.post(route('evaluator.applications.renewal.approve', application.value.id), {}, {
        preserveScroll: true,
...
const submitReject = () => {
    ...
    router.post(route('evaluator.applications.renewal.reject', application.value.id), { remarks: rejectForm.remarks }, {
        preserveScroll: true,
...
const resolveComplaint = (complaintId) => {
    if (confirm('Are you sure you want to mark this complaint as resolved?')) {
        router.post(route('evaluator.applications.renewal.resolve-complaint', [application.value.id, complaintId]), {}, { preserveScroll: true });
    }
};

const resolveRedFlag = (flagId) => {
    if (confirm('Are you sure you want to mark this red flag as resolved?')) {
        router.post(route('evaluator.applications.renewal.resolve-red-flag', [application.value.id, flagId]), {}, { preserveScroll: true });
    }
};
