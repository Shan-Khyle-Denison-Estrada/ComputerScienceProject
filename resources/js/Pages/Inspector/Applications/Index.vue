...
const fetchResults = debounce(() => {
    router.get(route('inspector.applications.index'), {
        search: search.value,
...
<template>
    <Head title="Pending Inspections - Inspector" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pending Renewals for Unit Inspection</h2>
        </template>
                    ...
                    <table class="w-full text-left border-collapse min-w-full">
                        <thead class="select-none">
                            <tr class="bg-gray-50 border-b">
                                ...
                                <th class="p-4 font-semibold text-sm text-gray-700 cursor-pointer hover:bg-gray-100 transition-colors" @click="sortBy('remarks')">
                                    <div class="flex items-center gap-1">
                                        Remarks
                                        <svg v-if="sortField === 'remarks' && sortDirection === 'asc'" class="w-4 h-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
                                        <svg v-else-if="sortField === 'remarks' && sortDirection === 'desc'" class="w-4 h-4 text-red-600 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
                                        <svg v-else class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"></path></svg>
                                    </div>
                                </th>
