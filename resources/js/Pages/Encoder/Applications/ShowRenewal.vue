...
// Uses inertia useForm for handling the final encoding inputs
...
const submitFinalize = () => {
    finalizeForm.post(route('encoder.applications.renewal.finalize', application.value.id), {
        preserveScroll: true,
...
<template>
    <Head title="Finalize Renewal" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Encoder Action: Finalize Renewal</h2>
                ...
                <div class="px-3 py-1 rounded-full text-sm font-semibold border"
                     :class="application.status === 'Completed' ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-green-100 text-green-700 border-green-200'">
                    {{ application.status }}
                </div>
            </div>
        </template>
        ...
        <div class="w-full flex flex-row gap-0 h-[calc(100vh-160px)] overflow-hidden relative">
            ...
            <div class="w-2/3 bg-white shadow-sm border-r border-gray-200 p-6 flex flex-col h-full flex-shrink-0">
                ...
                <div class="border-b border-gray-200 mb-6 flex gap-6 overflow-x-auto pb-1 flex-shrink-0">
                    ...
                    <button @click="activeTab = 'assessment'" :class="activeTab === 'assessment' ? 'border-blue-600 text-blue-600 border-b-2 font-semibold' : 'text-gray-500 font-medium hover:text-gray-700'" class="pb-2 whitespace-nowrap transition-colors">Assessment (Paid)</button>
                </div>
                    ...
                    <Transition name="fade" mode="out-in">
                        ...
                        <div v-else-if="activeTab === 'assessment'" class="space-y-6">
                            <div v-if="!application.assessment" class="p-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-300">
                                ...
                                <h3 class="text-lg font-medium text-gray-900">No Assessment Recorded</h3>
                            </div>
                            <div v-else class="bg-white border rounded-xl shadow-sm overflow-hidden border-green-200">
                                <div class="p-6 border-b bg-green-50 flex justify-between items-center">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-600 mt-1">Assessed on {{ application.assessment.assessment_date }}</p>
                                    </div>
                                    <span class="px-3 py-1 rounded-full text-sm font-bold bg-green-200 text-green-800 uppercase tracking-wide">
                                        {{ application.assessment.status }}
                                    </span>
                                </div>
                                <div class="p-6">
                                    <h4 class="font-semibold text-sm text-gray-700 mb-3 border-b pb-2">Payment History</h4>
                                    <div v-if="application.assessment.payments.length === 0" class="text-sm text-gray-500 italic text-center py-2">No payments recorded.</div>
                                    <div v-else class="space-y-3">
                                        <div v-for="(pay, index) in application.assessment.payments" :key="index" class="bg-gray-50 p-3 rounded border flex justify-between items-center">
                                            <div>
                                                <p class="text-sm font-bold text-gray-800">OR #: {{ pay.or_number }}</p>
                                                <p class="text-xs text-gray-500">{{ pay.date }} &bull; Payee: {{ pay.payee }}</p>
                                            </div>
                                            <span class="font-bold text-green-600">₱{{ parseFloat(pay.amount_paid).toFixed(2) }}</span>
                                        </div>
                                    </div>
                                    <div class="flex justify-between text-base font-bold pt-4 mt-4 border-t border-gray-200">
                                        <span class="text-gray-800">Total Cleared</span>
                                        <span class="text-green-600">₱{{ parseFloat(application.assessment.total_due).toFixed(2) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="w-1/3 bg-gray-50 p-8 flex flex-col h-full flex-shrink-0 border-l border-gray-200">
                <div class="flex flex-col items-center text-center">
                    ...
                    <p class="text-sm text-gray-500 mt-2">
                        This application has been fully approved. You may now encode the final details, set the issue date, and complete the renewal process.
                    </p>
                </div>

                <div class="pt-4 border-t border-gray-200 space-y-3 flex-shrink-0 mt-auto">
                    <PrimaryButton @click="showFinalizeModal = true" class="w-full justify-center py-3 bg-blue-600 hover:bg-blue-700 shadow text-sm">
        ...
        <Modal :show="showFinalizeModal" @close="showFinalizeModal = false" maxWidth="md">
            <div class="p-6">
                <h3 class="text-lg font-bold text-gray-900 mb-4 border-b pb-2 text-blue-600">Finalize Franchise Renewal</h3>
                <form @submit.prevent="submitFinalize">
                    <div class="mb-4">
                        <InputLabel value="New Date Issued" class="text-xs mb-1" />
                        <TextInput v-model="finalizeForm.new_date_issued" type="date" class="block w-full text-sm py-2" required />
                    </div>
                    <div class="mb-5">
                        <InputLabel value="Remarks / Notes (Optional)" class="text-xs mb-1" />
                        <textarea v-model="finalizeForm.remarks" class="w-full border-gray-300 focus:border-blue-500 rounded-md shadow-sm text-sm" rows="3" placeholder="Add any final encoding notes..."></textarea>
                    </div>
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <PrimaryButton type="submit" class="bg-blue-600 hover:bg-blue-700 focus:ring-blue-500" :disabled="finalizeForm.processing">
                            {{ finalizeForm.processing ? 'Processing...' : 'Complete Renewal' }}
                        </PrimaryButton>
                    </div>
                </form>
            </div>
