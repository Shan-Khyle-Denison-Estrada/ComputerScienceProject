...
const application = computed(() => {
    const app = props.application || {};
        ...
        // Fix: Use the applicant directly for New Franchises, fallback to franchise owner
        const currentUser = app.user || franchise.current_ownership?.new_owner?.user || {};
        const currentOperator = franchise.current_ownership?.new_owner || {};
        
        // Fix: Prioritize proposed_units since this is a New Franchise
        const proposedUnits = app.proposed_units || app.proposedUnits || [];
        const proposedUnit = proposedUnits.length > 0 ? proposedUnits[proposedUnits.length - 1] : null;
        
        const currentActiveUnit = franchise.current_active_unit || {};
        const fallbackUnit = currentActiveUnit.new_unit || {};
        
        const currentUnitData = proposedUnit || fallbackUnit;
        const currentMake = proposedUnit?.make || fallbackUnit.make || {};
        const currentZone = proposedUnit?.zone || app.zone || franchise.zone || {};
    ...
    return {
        ...
        franchise_details: {
            id: franchise.id,
            franchise_number: franchise.franchise_number || 'Pending',
            zone: currentZone.description || 'N/A',
            date_issued: franchise.date_issued ? new Date(franchise.date_issued).toLocaleDateString() : 'Pending',
        ...
        current_owner: {
                ...
                address: app.address || `${app.street_address || ''}, ${app.barangay || ''}, ${app.city || ''}, ${app.province || ''}`.replace(/^[,\s]+|[,\s]+$/g, '') || 'N/A',
            },
...
const submitApproval = () => {
    ...
    router.post(route('sp_approver.applications.renewal.approve', application.value.id), {}, {
        preserveScroll: true,
...
const submitReject = () => {
    ...
    router.post(route('sp_approver.applications.renewal.reject', application.value.id), { remarks: rejectForm.remarks }, {
        preserveScroll: true,
...
<template>
    <Head title="SP Review New Franchise" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">SP Review: New Franchise</h2>
        ...
        <div class="w-full flex flex-row gap-0 h-[calc(100vh-100px)] overflow-hidden relative rounded-lg">
                    ...
                    <Transition name="fade" mode="out-in">
                        ...
                        <div v-if="activeTab === 'franchise_overview'" class="space-y-8">
                            ...
                            <section>
                                ...
                                <div class="grid grid-cols-2 gap-y-4 gap-x-6">
                                    <div><p class="text-sm text-gray-500">Franchise Number</p><p class="font-medium text-gray-900">{{ application.franchise_details.franchise_number }}</p></div>
            ...
            <div class="w-1/3 bg-gray-50 border-r border-gray-200 flex flex-col h-full shadow-[inset_1px_0_5px_rgba(0,0,0,0.05)] flex-shrink-0">
                <div class="p-4 border-b border-gray-200 bg-white">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Verify departmental clearances prior to issuing final legislative approval.</p>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <div class="text-sm text-gray-500 bg-indigo-50 border border-indigo-100 p-4 rounded-lg">
                        <strong>Notice:</strong> As the Sangguniang Panlungsod Approver, you provide the final legislative/executive authorization for this franchise. Please verify that all departmental clearances and payments are completed before proceeding.
                    </div>
                </div>
                ...
                <div class="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <div v-if="application.status === 'Pending'" class="space-y-2">
                        <PrimaryButton @click="showApproveModal = true" class="w-full justify-center bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 py-3 text-base">Authorize Franchise</PrimaryButton>
                        <SecondaryButton @click="showRejectModal = true" class="w-full justify-center border-red-500 text-red-700 hover:bg-red-50 py-3 text-base">Reject Application</SecondaryButton>
                    </div>
        ...
        <div v-if="showApproveModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-sm overflow-hidden z-10">
                <div class="p-6">
                    <h2 class="text-lg font-bold text-indigo-600 mb-4">Authorize Franchise?</h2>
                    <p class="text-sm text-gray-500 mb-6">Approving this action provides the final legislative/executive authorization for the issuance of this franchise.</p>
                    <div class="flex justify-end gap-3">
                        <SecondaryButton @click="showApproveModal = false" :disabled="approveProcessing">Cancel</SecondaryButton>
                        <PrimaryButton @click="submitApproval" class="bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500" :disabled="approveProcessing">Confirm Authorization</PrimaryButton>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="showRejectModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden z-10">
                <div class="p-6">
                    ...
                    <p class="text-sm text-gray-500 mb-4">Please state the reason for denying SP authorization for this franchise application.</p>
                    <div class="mb-5">
                        <InputLabel value="Reason for Rejection" class="text-xs mb-1" />
                        <textarea v-model="rejectForm.remarks" class="w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm text-sm" rows="3" placeholder="Explain the grounds for denial..."></textarea>
                    </div>
