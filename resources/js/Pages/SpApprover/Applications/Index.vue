...
const fetchResults = debounce(() => {
    router.get(route('sp_approver.applications.index'), {
        search: search.value,
...
<template>
    <Head title="Pending Approvals - SP" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pending Renewals for SP Approval</h2>
        </template>
                        ...
                        <tbody>
                            <tr v-for="app in applications.data" :key="app.id" class="border-b hover:bg-gray-50 transition">
                                ...
                                <td class="p-4 text-sm">
                                    <Link v-if="app.application_type === 'Renewal'" :href="route('sp_approver.applications.show', app.id)" class="text-red-600 hover:text-red-800 font-medium text-sm">Review File &rarr;</Link>
                                    <Link v-else-if="app.application_type === 'New Franchise'" :href="route('sp_approver.applications.show-new-franchise', app.id)" class="text-red-600 hover:text-red-800 font-medium text-sm">Review File &rarr;</Link>
                                    <Link v-else :href="route('sp_approver.applications.show', app.id)" class="text-red-600 hover:text-red-800 font-medium text-sm">Review File &rarr;</Link>
                                </td>
                            </tr>
                            <tr v-if="applications.data.length === 0">
                                <td colspan="5" class="p-6 text-center text-gray-500">No verified applications requiring SP approval.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
