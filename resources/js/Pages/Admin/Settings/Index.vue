...
const props = defineProps({
    settings: {
        ...
        default: () => ({
            ...
            surcharge_rate: '',
            interest_rate: '',
            is_new_franchise_open: false, // <-- Added default prop
            theme_color: '#3B82F6',
            lgu_name: '',
            lgu_logo_path: null,
            office_name: '',
...
// --- DATE LOGIC ---
const initialStartMonth = props.settings.annual_renewal_start ? props.settings.annual_renewal_start.split('-')[0] : '01';
const initialStartDay = props.settings.annual_renewal_start ? props.settings.annual_renewal_start.split('-')[1] : '01';
const initialDueMonth = props.settings.annual_renewal_due ? props.settings.annual_renewal_due.split('-')[0] : '12';
const initialDueDay = props.settings.annual_renewal_due ? props.settings.annual_renewal_due.split('-')[1] : '31';

const selectedStartMonth = ref(initialStartMonth);
const selectedStartDay = ref(initialStartDay);
const selectedDueMonth = ref(initialDueMonth);
const selectedDueDay = ref(initialDueDay);
...
const form = useForm({
    ...
    is_new_franchise_open: !!props.settings.is_new_franchise_open, // <-- Added to form state
...
watch([selectedStartMonth, selectedStartDay], ([newMonth, newDay]) => {
    form.annual_renewal_start = `${newMonth}-${newDay}`;
});

watch([selectedDueMonth, selectedDueDay], ([newMonth, newDay]) => {
    form.annual_renewal_due = `${newMonth}-${newDay}`;
});
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="bg-gray-50 p-6 rounded-lg border border-gray-100 flex flex-col">
                                <div class="flex items-center gap-3 mb-4">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-gray-900 text-sm">Application Availability</h4>
                                        <p class="text-xs text-gray-500">Control public access to applications</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 space-y-4">
                                    <div class="flex items-center justify-between bg-white p-4 border border-gray-200 rounded-lg shadow-sm">
                                        <div class="mr-4">
                                            <span class="block text-sm font-bold text-gray-800">New Franchise Application</span>
                                            <span class="block text-xs text-gray-500 mt-1">When toggled on, users can submit applications for new franchises publicly.</span>
                                        </div>
                                        <label class="relative inline-flex items-center cursor-pointer shrink-0">
                                            <input type="checkbox" v-model="form.is_new_franchise_open" class="sr-only peer">
                            ...
                            <div class="bg-gray-50 p-6 rounded-lg border border-gray-100 flex flex-col lg:col-span-2">
                                <div class="flex items-center gap-3 mb-4">
