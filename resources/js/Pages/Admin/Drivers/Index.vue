...
const openEditModal = async (driver) => {
    ...
    editForm.contact_number = driver.contact_number; // Ensure you keep your formatContactNumber wrapper here if you used it
    ...
    editPhotos.user = driver.user_photo ? `/storage/${driver.user_photo}` : null;
    editPhotos.front = driver.license_front_photo ? `/storage/${driver.license_front_photo}` : null;
    editPhotos.back = driver.license_back_photo ? `/storage/${driver.license_back_photo}` : null;

    // OPEN MODAL IMMEDIATELY BEFORE AWAITING API
    showEditModal.value = true;

    // Pre-fetch locations in the background
    if (driver.province && provincesList.value.length) {
