...
// Add this watcher to trigger an alert when an error flash message arrives
watch(() => usePage().props.flash, (flash) => {
    if (flash && flash.error) {
        // alert(flash.error); 
        // Note: If you have a custom Toast or Modal component, 
        // you can trigger it here instead of a native alert.
    }
}, { deep: true });
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <transition name="fade">
            <div v-if="$page.props.flash.error" class="fixed bottom-5 right-5 z-[100] max-w-md bg-red-600 text-white px-6 py-4 rounded-lg shadow-2xl flex items-center gap-3">
                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <div class="flex-1">
                    <p class="font-bold">Action Failed</p>
                    <p class="text-sm opacity-90">{{ $page.props.flash.error }}</p>
                </div>
                <button @click="$page.props.flash.error = null" class="text-white hover:text-red-200">✕</button>
            </div>
        </transition>
