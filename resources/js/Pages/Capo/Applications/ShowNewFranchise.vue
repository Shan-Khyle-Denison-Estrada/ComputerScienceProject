...
<template>
    <Head title="CAPO Review New Franchise" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">CAPO Review: New Franchise</h2>
        ...
        <div class="w-full flex flex-row gap-0 h-[calc(100vh-100px)] overflow-hidden relative rounded-lg">
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4">
                    <Transition name="fade" mode="out-in">
                        ...
                        <div v-else-if="activeTab === 'inspections'" class="space-y-6">
                            <h3 class="font-bold text-gray-800 text-lg mb-4 flex items-center gap-2">Inspection Results</h3>
            ...
            <div class="w-1/3 bg-gray-50 border-r border-gray-200 flex flex-col h-full shadow-[inset_1px_0_5px_rgba(0,0,0,0.05)] flex-shrink-0">
                <div class="p-4 border-b border-gray-200 bg-white">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Review the inspector's physical unit inspection findings before granting final approval.</p>
                </div>
                
                <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    <div class="bg-white p-4 border rounded-lg shadow-sm">
                        <h4 class="font-semibold text-gray-800 mb-2 border-b pb-2">Prerequisite Checks</h4>
                    ...
                    <div class="text-sm text-gray-500 bg-blue-50 border border-blue-100 p-4 rounded-lg">
                        <strong>Notice:</strong> Approving this application signifies that the unit is fully compliant with anti-pollution guidelines and city ordinances. If anomalies are found, return the application to the inspector.
                    </div>
                </div>
                ...
                <div class="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <div v-if="application.status === 'Pending'" class="space-y-2">
                        <PrimaryButton v-if="can('capo_approve_applications')" @click="showApproveModal = true" class="w-full justify-center bg-green-600 hover:bg-green-700 py-3 text-base">Approve Application</PrimaryButton>
                        <SecondaryButton v-if="can('capo_reject_applications')" @click="showRejectModal = true" class="w-full justify-center border-red-500 text-red-700 hover:bg-red-50 py-3 text-base">Reject / Return to Inspector</SecondaryButton>
                    </div>
        ...
        <div v-if="showApproveModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-sm overflow-hidden z-10">
                <div class="p-6">
                    <h2 class="text-lg font-bold text-green-600 mb-4">Final Approval?</h2>
                    <p class="text-sm text-gray-500 mb-6">Are you sure you want to grant final approval for this new franchise?</p>
        ...
        <div v-if="showRejectModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden z-10">
                <div class="p-6">
                    <h2 class="text-lg font-bold text-red-600 mb-2">Return to Inspector</h2>
                    <p class="text-sm text-gray-500 mb-4">This will push the application back to the Inspector for re-evaluation.</p>
                    <div class="mb-5">
                        <InputLabel value="Reason for Return" class="text-xs mb-1" />
                        <textarea v-model="rejectForm.remarks" class="w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm text-sm" rows="3" placeholder="Explain what issues the inspector missed..."></textarea>
                    </div>
