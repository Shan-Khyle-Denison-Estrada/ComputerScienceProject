...
const handleSearch = debounce(() => {
    router.get(route('capo.applications.index'), {
        search: search.value,
...
<template>
    <Head title="Pending Approvals - CAPO" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pending Renewals for CAPO Approval</h2>
        </template>
                        ...
                        <tbody>
                            <tr v-for="app in applications.data" :key="app.id" class="border-b hover:bg-gray-50 transition">
                                ...
                                <td class="p-4 text-sm">
                                    <Link :href="route('capo.applications.show', app.id)" class="text-blue-600 hover:text-blue-800 font-medium text-sm">Review & Approve &rarr;</Link>
                                </td>
                            </tr>
