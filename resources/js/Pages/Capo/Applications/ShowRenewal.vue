                    ...
                    <Transition name="fade" mode="out-in">
                        ...
                        <div v-else-if="activeTab === 'unit_details'" class="space-y-6">
                            <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                                <h4 class="font-bold text-gray-800 text-md mb-4 border-b border-gray-200 pb-2">Franchise Unit Information</h4>
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-6 mb-8">
                                    <div><p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Make / Model</p><p class="font-medium text-base text-gray-900">{{ application.current_unit.make }}</p></div>
                                    <div><p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Year</p><p class="font-medium text-base text-gray-900">{{ application.current_unit.year }}</p></div>
                                    <div><p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Motor No.</p><p class="font-medium text-base text-gray-900">{{ application.current_unit.motor_no }}</p></div>
                                    <div><p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Chassis No.</p><p class="font-medium text-base text-gray-900">{{ application.current_unit.chassis_no }}</p></div>
                                    <div><p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Plate Number</p><p class="font-medium text-base text-gray-900">{{ application.current_unit.plate_no }}</p></div>
                                    <!-- <div><p class="text-xs text-gray-500 uppercase tracking-wider mb-1">CR Number</p><p class="font-medium text-base text-gray-900">{{ application.current_unit.cr_no }}</p></div> -->
                                </div>
                                <h5 class="text-sm font-bold text-gray-700 uppercase mb-4">Unit Photos</h5>
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div v-for="view in unitViews" :key="view.key" class="border border-gray-200 rounded-lg p-2 bg-white shadow-sm">
