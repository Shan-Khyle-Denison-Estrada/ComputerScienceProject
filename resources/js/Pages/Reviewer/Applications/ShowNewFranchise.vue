...
const application = computed(() => {
    ...
    // For a New Franchise, there is no existing franchise data yet. 
    // Data comes directly from the Application and ProposedUnit.
    const appUser = app.user || {};
    
    // Get the latest proposed unit
    ...
    const proposedZone = proposedUnit.zone || {};
    ...
    return {
        ...
        franchise_details: {
            id: 'N/A',
            zone: proposedZone.description || 'N/A',
            date_issued: 'Pending',
            mtfrb_case_no: 'Pending',
        },
        
        current_owner: {
            first_name: app.first_name || appUser.first_name || 'Not specified',
            last_name: app.last_name || appUser.last_name || 'Not specified',
            contact: app.contact_number || appUser.contact_number || 'N/A',
            email: app.email || appUser.email || 'N/A',
...
<template>
    <Head title="Final Review New Franchise" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Final Review: New Franchise</h2>
        ...
        <div class="w-full flex flex-row gap-0 h-[calc(100vh-100px)] overflow-hidden relative rounded-lg">
            ...
            <div class="w-1/3 bg-gray-50 border-r border-gray-200 flex flex-col h-full shadow-[inset_1px_0_5px_rgba(0,0,0,0.05)] flex-shrink-0">
                <div class="p-4 border-b border-gray-200 bg-white">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Ensure all departments have cleared this franchise and all fees are fully paid.</p>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <div class="text-sm text-gray-500 bg-indigo-50 border border-indigo-100 p-4 rounded-lg">
                        <strong>Notice:</strong> As a Reviewer, your role is to validate the preceding steps. Please confirm that the Evaluator, Inspector, and CAPO have approved the application, and verify that any applicable assessments and payments are fully cleared.
                    </div>
                </div>
                ...
                <div class="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <div v-if="application.status === 'Pending'" class="space-y-2">
                        <PrimaryButton v-if="can('reviewer_approve_applications')" @click="showApproveModal = true" class="w-full justify-center bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 py-3 text-base">Approve Application</PrimaryButton>
                        <SecondaryButton v-if="can('reviewer_reject_applications')" @click="showRejectModal = true" class="w-full justify-center border-red-500 text-red-700 hover:bg-red-50 py-3 text-base">Reject Application</SecondaryButton>
                    </div>
        ...
        <div v-if="showApproveModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-sm overflow-hidden z-10">
                <div class="p-6">
                    <h2 class="text-lg font-bold text-indigo-600 mb-4">Final Sign-off?</h2>
                    <p class="text-sm text-gray-500 mb-6">Approving this application will mark it as fully reviewed and finalized.</p>
                    <div class="flex justify-end gap-3">
                        <SecondaryButton @click="showApproveModal = false" :disabled="approveProcessing">Cancel</SecondaryButton>
                        <PrimaryButton @click="submitApproval" class="bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500" :disabled="approveProcessing">Confirm Approval</PrimaryButton>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="showRejectModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden z-10">
                <div class="p-6">
                    ...
                    <p class="text-sm text-gray-500 mb-4">Provide the primary reason for rejecting this completely processed application.</p>
                    <div class="mb-5">
